...
class Live extends Controller
{
    ...
    {
        if($path=='') return json(['code'=>1, 'msg'=>'缺少参数', 'data'=>[]]);
    ...
    public function applyLive()
    {
        ...
        if (true !== $result) return json(['code'=>0, 'msg'=>$result, 'data'=>[]]);
        if($id>0){
    ...
    public function appointmentRoom()
    {
        ...
        if(empty($room_info)) return json(['code'=>0, 'msg'=>'直播间信息不存在', 'data'=>[]]);
    ...
    public function getRoomList()
    {
        ...
        $url = 'http://api.weixin.qq.com/wxa/business/getliveinfo?access_token=' . $token['access_token'];
        $page = input('page', 0);
        $limit = 20;
        ...
        if ($res['errcode'] == 1) {
            ...
            return json(['code' => 1, 'msg' => 'success', 'data' => $room_info]);
        }
    }
