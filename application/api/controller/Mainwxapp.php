   ...
   //多规格栏目列表页获取列表信息
    ...
    public function doPagegetmygwc(){
     ...
     $replaceRes = [];
      ...
      foreach ($express_data['normal'] as $item ) {
            $item['pro_info'] = str_replace(',','', $item['pro_info']);
            array_push($replaceRes,$item);
         ///   dump($express_data['normal']);
      }


      $express_data['normal']  = $replaceRes;
      //dump($express_data['normal']);
    //   for($i = 0; $i<count($express_data['mormal']);$i++){
...
//        $buydata = ['129|1554|1', '133|1579|2', '128|-1|5', '127|1550|3']; //产品id|规格值id(-1为单规格)|数量
    ...
    public function doPageGetProModel(){
        ...
        //获取分类
        $cate = Db::name('wd_xcx_cate_pro')->where('uniacid', $uniacid)->where('pid', $pid)->column('cate_id');
        $cate_info = Db::name('wd_xcx_cate')->where('id', 'in', $cate)->field('limit_start_time,limit_end_time')->select();
        $now_time = date('Hi', time());
        foreach ($cate_info as $vol){
            if($vol['limit_start_time'] && $vol['limit_end_time']){
                if($now_time<str_replace(':','',trim($vol['limit_start_time'])) || $now_time>=str_replace(':','', trim($vol['limit_end_time']))){
                    return json_encode(array('error'=>4, 'msg'=>'不在售卖时间内'));
                    break;
                }
            }
        }
