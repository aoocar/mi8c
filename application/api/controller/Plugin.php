...
class Plugin extends Controller
{
    ...
    public function ddlb()
    {
        ...
        $c = Db::name('wd_xcx_main_shop_order_item')->where("uniacid", $uniacid)->order('complete_time', 'desc')->limit(5)->field('suid, complete_time')->select();
                ...
                if ($data['forminfo']['tp_text']) {
                    foreach ($data['forminfo']['tp_text'] as $key => &$res) {
                        if ($key > 0) {
                            ...
                            } elseif ($res['field_type'] == '多行文本') {
                                $tp_text[$tp_key]['type'] = 1;
                                $tp_text[$tp_key]['tp_text'] = '';
                            } elseif ($res['field_type'] == '多选' || $res['field_type'] == '单选') {
    ...
    public function xnlf()
    {
        ...
        Db::name('wd_xcx_base')->where("uniacid", $uniacid)->update($num);
    ...
    {
        $uniacid = input('uniacid');
        $systemSet = Db::name('wd_xcx_shops_set')->where("uniacid", $uniacid)->find();
 
        if ($systemSet['is_filter_shop'] == 1) {
            //判断地埋位置过滤是否开启
            $ip = get_client_ip();
      
            $url = 'https://apis.map.qq.com/ws/location/v1/ip?key=2EBBZ-KFI2P-75LDD-LZVBH-OWEDK-ZVBL3&ip='.$ip;
            $location_text = json_decode(http_request($url), true);
            //   dump($location_text);
            $map = '';
            if ($location_text['status']==0) {
                $city = $location_text['result']['ad_info']['city'];
                $map .= ' and `city`="'. $city .'" ';
            }
        }
        ...
        $store['storeHot'] = Db::query("SELECT id,uniacid,name,logo,hot FROM {$prefix}wd_xcx_shops_shop WHERE `status` = 1 and `flag` = 1 AND `uniacid` = {$uniacid} AND `hot` = 1 {$map} ORDER BY " . $orderby . " LIMIT 0," . $tjnum);
