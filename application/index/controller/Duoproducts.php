...
class Duoproducts extends Controller
{
    ...
    public function cateadd()
    {
        if (check_login()) {
            if (powerget()) {
                ...
                $cateinfo['discount_data'] = unserialize($cateinfo['discount_data']);
    ...
    public function catesave()
    {
        ...
        //分类优惠价格
        // $cate_discounts_price = input('cate_discounts_price');

        // $data['cate_discounts_price'] = $cate_discounts_price;
        ...
        //              商户对每个商品分类的折扣进行设置
        // $discount_data = [
        //     'motorcycleA' => input('motorcycleA'),     //摩托车配件
        //     'travelA' => input('travelA'),   //行驶配件
        //     'structralA' => input('structralA'), //结构配件
        //     'maintenanceA' => input('maintenanceA'), //维修耗材
        // ];
        //列表类型
        ...
        //  $data['discount_data'] = serialize($discount_data);
        ...
        if (check_login()) {
            if (powerget()) {
                ...
                if ($cid > 0) {
                    ...
                    if ($keys && $is_top) {
                        $product = Db::name('wd_xcx_products')->alias('a')->join('wd_xcx_cate_pro b', 'a.id = b.pid', 'LEFT')->where('a.uniacid', $appletid)->where('a.type', 'showProMore')->where('b.cate_id', 'in', $cate_arr)->where('a.is_more', 3)->where('a.title', 'like', '%'.$keys.'%')->group("a.id")->order('a.num desc, a.id desc')->field('a.*')->paginate(10, false, [ 'query' => array('appletid'=>input("appletid"), 'cid'=>$cid, 'key'=>input('key'))]);
                        $count = Db::name('wd_xcx_products')->alias('a')->join('wd_xcx_cate_pro b', 'a.id = b.pid', 'LEFT')->where('a.uniacid', $appletid)->where('a.type', 'showProMore')->where('b.cate_id', 'in', $cid)->where('a.is_more', 3)->where('a.title', 'like', '%'.$keys.'%')->group("a.id")->field('a.id')->count();
                    } elseif (!$keys && $is_top) {
                        $product = Db::name('wd_xcx_products')->alias('a')->join('wd_xcx_cate_pro b', 'a.id = b.pid', 'LEFT')->where('a.uniacid', $appletid)->where('a.type', 'showProMore')->where('b.cate_id', 'in', $cate_arr)->where('a.is_more', 3)->group("a.id")->order('a.num desc, a.id desc')->field('a.*')->paginate(10, false, [ 'query' => array('appletid'=>input("appletid"), 'cid'=>$cid)]);
                        $count = Db::name('wd_xcx_products')->alias('a')->join('wd_xcx_cate_pro b', 'a.id = b.pid', 'LEFT')->where('a.uniacid', $appletid)->where('a.type', 'showProMore')->where('b.cate_id', 'in', $cid)->where('a.is_more', 3)->group("a.id")->field('a.id')->count();
                    } elseif ($keys && !$is_top) {
                        $product = Db::name('wd_xcx_products')->alias('a')->join('wd_xcx_cate_pro b', 'a.id = b.pid', 'LEFT')->where('a.uniacid', $appletid)->where('a.type', 'showProMore')->where('b.cate_id', $cid)->where('a.is_more', 3)->where('a.title', 'like', '%'.$keys.'%')->group("a.id")->order('a.num desc, a.id desc')->field('a.*')->paginate(10, false, [ 'query' => array('appletid'=>input("appletid"), 'cid'=>$cid, 'key'=>input('key'))]);
                        $count = Db::name('wd_xcx_products')->alias('a')->join('wd_xcx_cate_pro b', 'a.id = b.pid', 'LEFT')->where('a.uniacid', $appletid)->where('a.type', 'showProMore')->where('b.cate_id', $cid)->where('a.is_more', 3)->where('a.title', 'like', '%'.$keys.'%')->group("a.id")->field('a.id')->count();
                    } elseif (!$keys && !$is_top) {
                        $product = Db::name('wd_xcx_products')->alias('a')->join('wd_xcx_cate_pro b', 'a.id = b.pid', 'LEFT')->where('a.uniacid', $appletid)->where('a.type', 'showProMore')->where('b.cate_id', $cid)->where('a.is_more', 3)->group("a.id")->order('a.num desc, a.id desc')->field('a.*')->paginate(10, false, [ 'query' => array('appletid'=>input("appletid"), 'cid'=>$cid)]);
                        $count = Db::name('wd_xcx_products')->alias('a')->join('wd_xcx_cate_pro b', 'a.id = b.pid', 'LEFT')->where('a.uniacid', $appletid)->where('a.type', 'showProMore')->where('b.cate_id', $cid)->where('a.is_more', 3)->group("a.id")->field('a.id')->count();
                    }
                } else {
                    if ($keys != '') {
                        $product = Db::name('wd_xcx_products')->alias('a')->join('wd_xcx_cate_pro b', 'a.id = b.pid', 'LEFT')->where('a.uniacid', $appletid)->where('a.type', 'showProMore')->where('a.title', 'like', '%'.$keys.'%')->where('a.is_more', 3)->group("a.id")->order('a.num desc, a.id desc')->field('a.*')->paginate(10, false, [ 'query' => array('appletid'=>input("appletid"), 'key'=> $keys)]);
                        $count = Db::name('wd_xcx_products')->alias('a')->join('wd_xcx_cate_pro b', 'a.id = b.pid', 'LEFT')->where('a.uniacid', $appletid)->where('a.type', 'showProMore')->where('a.title', 'like', '%'.$keys.'%')->where('a.is_more', 3)->group("a.id")->count();
                    } else {
                        $product = Db::name('wd_xcx_products')->alias('a')->join('wd_xcx_cate_pro b', 'a.id = b.pid', 'LEFT')->where('a.uniacid', $appletid)->where('a.type', 'showProMore')->where('a.is_more', 3)->group('a.id')->order('a.num desc, a.id desc')->field('a.*')->paginate(10, false, [ 'query' => array('appletid'=>input("appletid"))]);
                        $count = Db::name('wd_xcx_products')->alias('a')->join('wd_xcx_cate_pro b', 'a.id = b.pid', 'LEFT')->where('a.uniacid', $appletid)->where('a.type', 'showProMore')->where('a.is_more', 3)->group('a.id')->count();
                    }
                }
            ...
            if (powerget()) {
                ...
                if ($status > 0) {
                    if ($status == 1) { //前台选择 待付款
                        $where['status'] = 0;
                    } elseif ($status == 2) { //前台选择 待发货
                        $where['status'] = 1;
                    } elseif ($status == 3) { //前台选择 待收货
                        $where['status'] = 2;
                        $where['delivery_type'] = 1;
                    } elseif ($status == 4) { //前台选择 待核销
                        $where['status'] = 2;
                        $where['delivery_type'] = 2;
                    } elseif ($status == 5) { //前台选择 交易成功
                        $where['status'] = 5;
                    } elseif ($status == 6) { //前台选择 交易关闭
                ...
                } elseif (!empty($screen_starttime)) {
                    $where['creat_time'] = ['>=', strtotime($screen_starttime)];
                } elseif (!empty($screen_endtime)) {//下单时间结束
                        ...
                        if ($pay_to == 1) {//支付到   1 微信
                            ...
                            if ($res) {
                                if ($res['return_code'] == 'SUCCESS') {
                                    ...
                                    $this->error('退款失败!请检查系统设置->QQ小程序设置');
                                    exit;
                                }
                            } else {
                                $this->error('退款失败!请检查系统设置->QQ小程序设置');
            ...
            if ($count > 1) {
                ...
                foreach ($orderItems as $ki => $vi) {
                    ...
                    if ($vi['status'] == -4 || $vi['status'] == -3 || $vi['status'] == -2 || $vi['status'] == -1) {
                        ...
                        $status = '待发货';
                    } elseif ($vi['status'] == 2 && $vi['delivery_type'] == 1) {
                        $status = '待收货';
                    } elseif ($vi['status'] == 2 && $vi['delivery_type'] == 2) {
                        $status = '待核销';
                    } elseif ($vi['status'] == 3 && $vi['delivery_type'] == 1) {
                        $status = '已收货';
                    } elseif ($vi['status'] == 3 && $vi['delivery_type'] == 2) {
                        $status = '已核销';
                    } elseif ($vi['status'] == 4 || $vi['status'] == 5) {
                        $status = '退款中';
                    } elseif ($vi['status'] == 6) {
                        $status = '退款退货中';
                    } elseif ($vi['status'] == 7) {
    ...
    public function servicedown()
    {
        ...
        foreach ($service_orders as $k => $v) {
            ...
            if ($count > 1) {
                ...
                if ($v['status'] == 0) {
                    $status = '待处理';
                } elseif ($v['status'] == -1 || $v['status'] == 1 || $v['status'] == 2 || $v['status'] == 3) {
    ...
    //需要使用证书的请求
    public function postXmlSSLCurl($xml, $url, $second=30, $uniacid)
    {
        $ch = curl_init();
        //超时时间
