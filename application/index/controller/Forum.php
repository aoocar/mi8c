    ...
    public function banner()
    {
        if(check_login()){
            if(powerget()){
                ...
                $carousel = Db::name('wd_xcx_forum_carousel')->where('appletid',$appletid)->select();
                foreach($carousel as $item){
                    $this->assign('carousel',$item);
                }
    ...
    public function bannerSave()
    {
        if(check_login()){
            if(powerget()){
                ...
                $carouselImg = $_POST['commonuploadpic3'];
                if($carouselImg){
                    $data['appletid'] = $id;
                    $data['carousel'] = remote($appletid,$carouselImg,2);
                    //echo $data['carouselImg'];
                    $res = Db::name('wd_xcx_forum_carousel')->insert($data);
                    if($res){
                        return $this->success('添加成功','/index/forum/banner.html?appletid='.$id);
                    }else{
    ...
    public function bannerDel()
    {
        if(\check_login()){
            if(\powerget()){
                ...
                $res = Db::name('wd_xcx_forum_carousel')->where('id',$carousel_id)->delete();
                if($res){
