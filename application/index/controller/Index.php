...
class Index extends Base
{
    ...
    public function save(){
        ...
        $data['anchor_audit_type'] = input("anchor_audit_type");
        $data['is_force_login'] = input("is_force_login");
        $data['is_force_phone'] = input("is_force_phone");
...
//                include_once 'Ordinary.php';
    ...
    public function geturl(){
        ...
        if($domain['domain']){
            ...
            return 'http://'.$_SERVER['HTTP_HOST'].'/front/index/index?uniacid='.$uniacid;
        }
    }
    ...
    public function getqrcode(){
        ...
        if($pageid){
            $url = 'https://www.'.$_SERVER['SERVER_NAME']. STATIC_ROOT . '/h5/index.html?id='.$uniacid.'#/pages/index/index?pageid='.$pageid;
        }else{
            $url = 'https://www.'.$_SERVER['SERVER_NAME']. STATIC_ROOT . '/h5/index.html?id='.$uniacid;
        }
...
//         dump($filename);die;
    ...
    public function downloadqq(){
        ...
        //远程服务器更改配置, 打包, 返回文件名
        $url = "https://qdy.hdewm.com/qqprogram.php?uniacid=$uniacid&domain=$domain&ip=$ip&platform=$platform";
        $response = $this->_requestGetcurl($url);
        //下载到服务器
        $url = 'https://qdy.hdewm.com/qqzip/'.$response;
