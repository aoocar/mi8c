...
class Powerfulsh extends Base
{
    ...
    {
        $upimages = '(
            "https://www.myyooo.com/upimages/20190724/7df92f6080f7ab70db27867d9e34d64b257.jpg",
            "https://www.myyooo.com/upimages/20190724/a612515d24800d628d5301d2d364cea2620.jpg"
        )';
    ...
    public function tenantsave()
    {
        ...
        if (input("latitude") && input("longitude")) {
            $url = 'https://apis.map.qq.com/ws/geocoder/v1/?key='. config('tx_map_key') .'&location='. input("latitude") .','. input("longitude");
            $location_text = json_decode(http_request($url), true);
            if ($location_text['status']==0) {
                $province = $location_text['result']['address_component']['province'];
                $city = $location_text['result']['address_component']['city'];
                $district = $location_text['result']['address_component']['district'];
            }
        }
...
//                    //子集数据量
            ...
            if (powerget()) {
                ...
                if ($op == 'quxiao' || $op == 'allowth' || $op == 'confirmtk') {   //取消订单  同意退货  同意取消订单
                    ...
                    try {
                        ...
                        if ($op == 'quxiao') {
                            $data['flag'] = 5;
                        } elseif ($op == 'allowth'|| $op == 'confirmtk') {
                            $data['flag'] = 8;
                        }
                ...
                if ($op == 'delorder') {
                    ...
                    Db::name("wd_xcx_evaluate")->where('orderid', $orderid)->delete();
                    $res = Db::name('wd_xcx_duo_products_order')->where("order_id", $orderid)->delete();
                ...
                foreach ($orders as $key => &$res) {
                    ...
                    if ($res['hxinfo']) {
                        if ($res['hxinfo'][0] == 1) {
                            $res['hxinfo2'] = "系统核销";
                        } elseif ($res['hxinfo'][0] == '密码核销' || $res['hxinfo'][0] == '管理员核销') {
                            $res['hxinfo2'] = $res['hxinfo'][0];
                        } elseif ($res['hxinfo'][0]=='核销员核销') {
    ...
    public function systemsave()
    {
        ...
        $data['is_filter_shop'] = input('is_filter_shop');
    ...
    public function _Postrequest($url, $data, $ssl = true)
    {
        $headers = [
